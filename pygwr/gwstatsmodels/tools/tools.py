...
from gwstatsmodels.distributions import (ECDF, monotone_fn_inverter,
                                               StepFunction)
from gwstatsmodels.tools.data import _is_using_pandas
...
def add_constant(data, prepend=False):
    ...
    .. WARNING::
       ...
       gwstatsmodels. We recommend to use an explicit prepend in any permanent
       code.
    '''
    if not prepend:
        import inspect
...
StepFunction = np.deprecate(StepFunction,
                old_name = 'gwstatsmodels.tools.tools.StepFunction',
                new_name = 'gwstatsmodels.distributions.StepFunction')
monotone_fn_inverter = np.deprecate(monotone_fn_inverter,
                old_name = 'gwstatsmodels.tools.tools.monotone_fn_inverter',
                new_name = 'gwstatsmodels.distributions.monotone_fn_inverter')
ECDF = np.deprecate(ECDF,
                old_name = 'gwstatsmodels.tools.tools.ECDF',
                new_name = 'gwstatsmodels.distributions.ECDF')
...
def chain_dot(*arrs):
    ...
    -------
    >>> import numpy as np
    >>> from gwstatsmodels.tools import chain_dot
...
def webuse(data, baseurl='http://www.stata-press.com/data/r11/'):
    ...
    # lazy imports
    import pandas
    from gwstatsmodels.iolib import genfromdta
